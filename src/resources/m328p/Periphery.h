...
namespace Periphery {
	...
	static Usart& usart0 = *(reinterpret_cast<Usart*>(ADR_USART));
	...
	inline Usart& getUsart() noexcept {
		return *(reinterpret_cast<Usart*>(ADR_USART));
	}
	...
	template <> inline constexpr uint8_t getCapacity<Port>() noexcept { return 3; }
	template <> inline constexpr uint8_t getCapacity<Usart>() noexcept { return 1; }
